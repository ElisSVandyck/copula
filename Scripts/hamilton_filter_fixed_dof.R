...
bac_raw <- read.csv(file = "C:/Users/eliss/Documents/StatsUCL/xSummer Project/Data/bac.csv")
c_raw <- read.csv(file = 'C:/Users/eliss/Documents/StatsUCL/xSummer Project/Data/c.csv')
gs_raw <- read.csv(file = 'C:/Users/eliss/Documents/StatsUCL/xSummer Project/Data/gs.csv')
ms_raw <- read.csv(file = 'C:/Users/eliss/Documents/StatsUCL/xSummer Project/Data/ms.csv')
jpm_raw <- read.csv(file = 'C:/Users/eliss/Documents/StatsUCL/xSummer Project/Data/jpm.csv')
...
theta_comb <- c(theta_1[1:3],theta_2[1:3], dof)
...
theta_comb_P <- c(theta_comb, p11, p22)
...
hamilton_filter_map <- function(theta_plus_P_input){
  ...
  fyt_store <- rep(0, n)
  #Infinite mappings
  # Potentially insert code here
  
  #Update parameters
  input_par_1 <-  matrix(c(0, theta_plus_P_input[1], theta_plus_P_input[2], 0, 0, 
                           theta_plus_P_input[3], 0, 0, 0), 3, 3)
  input_par_2 <-  matrix(c(0, theta_plus_P_input[4], theta_plus_P_input[5], 0, 0, 
                           theta_plus_P_input[6], 0, 0, 0), 3, 3)
  input_dof <- matrix(c(0, theta_plus_P_input[7], theta_plus_P_input[7], 0, 0, 
                        theta_plus_P_input[7], 0, 0, 0), 3, 3)
  
  vine_structure_update_1 <- RVineMatrix(Matrix = structure_matrix, family = family_matrix, 
                                         par = input_par_1, par2 = input_dof)
  vine_structure_update_2 <- RVineMatrix(Matrix = structure_matrix, family = family_matrix, 
                                         par = input_par_2, par2 = input_dof)
  p11 <- theta_plus_P_input[8]
  p22 <- theta_plus_P_input[9]
  print(p11)
  print(p22)
  for(i in 2:n){
...
stats::optim(par = theta_comb_P, fn = hamilton_filter_map, method = 'Nelder-Mead',
